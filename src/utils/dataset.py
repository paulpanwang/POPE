...
def convert_gray(image, augment_fn=None, client=SCANNET_CLIENT):
...
# --- ScanNet ---
def read_scannet_rgb(path, resize=(448, 448), augment_fn=None):
    """
    Args:
...
def read_scannet_grayv2(path, augment_fn=None):
    """
    Args:
        resize (tuple): align image to depthmap, in (w, h).
        augment_fn (callable, optional): augments images with pre-defined visual effects
