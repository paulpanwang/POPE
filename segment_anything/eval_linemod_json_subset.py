...
def recall_object(boxA, boxB, thresholded=0.5):
    ...
    recall_acc = interArea / boxBArea
    return iou, recall_acc
...
for label_idx , test_dict in enumerate(dir_list):
    ...
    all_is_recalled, all_recall_acc = 0.,0.
    for rotation_key, rotation_list in zip(test_dict.keys(), test_dict.values()):
        for pair_idx,pair_name in enumerate(tqdm(rotation_list[::10])):
            ...
            similarity_score, top_images  = np.array([0],np.float32) , [[]]
            t3 = time.time()
            compact_percent = 0.3
            ...
            is_recalled,recall_acc = recall_object(pre_bbox , gt_bbox)
            all_is_recalled += float(is_recalled)
            all_recall_acc +=float(recall_acc)
    ...
    val_metrics_4tb["mask_miou"] = float(all_is_recalled/all_image)
    val_metrics_4tb["mask_precision"] = float(all_recall_acc/all_image)
