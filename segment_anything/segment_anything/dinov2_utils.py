...
import sys
sys.path.insert(0, "/mnt/bn/pico-panwangpan-v2/ObjRelPose/")
...
def load_dinov2_model():
    ROOT_DIR = "/mnt/bn/pico-panwangpan-v2/ObjRelPose/"
    ...
    # cfg = get_cfg(f"{ROOT_DIR}/dinov2/dinov2/configs/eval/vits14_pretrain.yaml")
    cfg = get_cfg(f"{ROOT_DIR}/dinov2/dinov2/configs/eval/vits14_pretrain.yaml")
    model, _, embed_dim = build_model_from_cfg(cfg, only_teacher=False)
    # model, embed_dim = build_model_from_cfg(cfg, only_teacher=True)
    load_pretrained_weights(model, f'{ROOT_DIR}/assets/dinov2_vits14_pretrain.pth', checkpoint_key="student")
    # load_pretrained_weights(model, f'{ROOT_DIR}/assets/dinov2_vitl14_pretrain.pth', checkpoint_key="teacher")
    model.eval()
    return model
