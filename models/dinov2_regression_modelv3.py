...
class DINOv2Poser(nn.Module):
    def __init__(self, default_cfg, depth=4, heads=8, dim_head = 64, dropout = 0., emb_dropout = 0., mlp_dim=512):
        ...
        self.MLP1 = FeedForward( dim = 2048, hidden_dim = 1024, num_classes = 512, dropout = 0.1)
        self.MLP2 = FeedForward( dim = 2048, hidden_dim = 1024, num_classes = 512, dropout = 0.1)
        self.cross_attentionAll = CrossAtten(default_cfg['coarse'], token_dim, ['cross']*2 )
        self.MLP = FeedForward( dim = 1024, hidden_dim = 512, num_classes = 1, dropout = 0.1)
    ...
    def forward(self, data, only_att_fea=True , use_avg = False):
        ...
        all_tokensA = torch.cat([feaA, cls_tokenA],dim=2)
        all_tokensB = torch.cat([feaB, cls_tokenB],dim=2)
        all_tokensA = self.MLP1(all_tokensA)
        all_tokensB = self.MLP2(all_tokensB)
        cross_A, cross_B = self.cross_attentionAll(all_tokensA, all_tokensB) 
        ensemble = torch.cat((cross_A, cross_B), dim=2)
